...
class WBTool(ActionListener):
 ...
 def _build_ui(self):
  self.frame = JFrame("GelAnno")
  self.frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE)
  self.frame.setLayout(BorderLayout())
  ctrl = JPanel()
  ctrl.setLayout(BoxLayout(ctrl, BoxLayout.Y_AXIS))
  ctrl.setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8))
  ctrl.setPreferredSize(Dimension(200, 800))
  def section(t):
   lbl = JLabel(t)
   lbl.setFont(Font("Arial", Font.BOLD, 10))
   lbl.setAlignmentX(0.0)
   ctrl.add(lbl);  ctrl.add(JSeparator())
   ctrl.add(Box.createVerticalStrut(2))
  def btn(label, cmd):
   b = JButton(label)
   b.setActionCommand(cmd)
   b.addActionListener(self)
   b.setAlignmentX(0.0)
   b.setMaximumSize(Dimension(190, 22))
   ctrl.add(b);  ctrl.add(Box.createVerticalStrut(1))
   return b
  section("Image")
  ...
  lsp.setPreferredSize(Dimension(185, 55))
  lsp.setMaximumSize(Dimension(190, 55))
  lsp.setAlignmentX(0.0)
  ctrl.add(lsp);  ctrl.add(Box.createVerticalStrut(2))
  btn("Move Up",    "band_up")
  ...
  dpad.setMaximumSize(Dimension(90, 45))
  ...
  size_pad.setMaximumSize(Dimension(90, 20))
  ...
  ctrl.add(JSeparator());  ctrl.add(Box.createVerticalStrut(2))
  ctrl.add(self.status_label)
  self.fig_canvas = FigureCanvas(self)
  self.fig_scroll = JScrollPane(self.fig_canvas)
  self.fig_scroll.setPreferredSize(Dimension(750, 800))
  ctrl_scroll = JScrollPane(ctrl)
  ctrl_scroll.setHorizontalScrollBarPolicy(
   JScrollPane.HORIZONTAL_SCROLLBAR_NEVER)
  ctrl_scroll.setVerticalScrollBarPolicy(
   JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED)
  ctrl_scroll.setPreferredSize(Dimension(220, 800))
  ctrl_scroll.getVerticalScrollBar().setUnitIncrement(16)
  self.frame.add(ctrl_scroll,     BorderLayout.WEST)
  self.frame.add(self.fig_scroll, BorderLayout.CENTER)
  self.frame.pack()
  bounds = awt.GraphicsEnvironment.getLocalGraphicsEnvironment() \
   .getMaximumWindowBounds()
  self.frame.setLocation(bounds.x, bounds.y)
  self.frame.setSize(min(self.frame.getWidth(), bounds.width // 2),
                           min(self.frame.getHeight(), bounds.height))
  def zoom(c, f):
   font = c.getFont()
   if font:
    c.setFont(font.deriveFont(float(max(7,
     min(18, round(font.getSize2D() * f))))))
   for name in ("Preferred", "Maximum"):
    d = getattr(c, "get" + name + "Size")()
    if d and d.width < 10000 and d.height < 10000:
     getattr(c, "set" + name + "Size")(Dimension(
      max(1, int(d.width * f)), max(1, int(d.height * f))))
   for child in c.getComponents(): zoom(child, f)
   ctrl.revalidate(); ctrl.repaint()
  class ZoomAction(AbstractAction):
   def __init__(self, f): self.f = f
   def actionPerformed(self, event): zoom(ctrl, self.f)
  root = self.frame.getRootPane()
  imap = root.getInputMap(JPanel.WHEN_IN_FOCUSED_WINDOW)
  amap = root.getActionMap()
  meta = awt.event.InputEvent.META_DOWN_MASK
  for key, name, factor in ((KeyEvent.VK_MINUS, "zoom-out", .9),
                                  (KeyEvent.VK_EQUALS, "zoom-in", 1/.9)):
   imap.put(KeyStroke.getKeyStroke(key, meta), name)
   amap.put(name, ZoomAction(factor))
  imap.put(KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, meta |
   awt.event.InputEvent.SHIFT_DOWN_MASK), "zoom-in")
